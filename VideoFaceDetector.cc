// Most of the code here is by Mateo Hrastnik and has been borrowed
// from this repository (MIT License):
//   https://github.com/hrastnik/face_detect_n_track
//
// There are some modifications by Lari Lampen.
...
void VideoFaceDetector::setFaceFound()
{
  if (!m_foundFace) {
    m_foundFace = true;
  }
...
void VideoFaceDetector::setFaceLost()
{
  if (m_foundFace) {
    m_foundFace = false;
  }
...
void VideoFaceDetector::detectFaceAllSizes(const cv::Mat &frame)
{
  ...
  m_faceCascade->detectMultiScale(frame, m_allFaces, 1.1, 10, 0,
...
void VideoFaceDetector::detectFacesTemplateMatching(const cv::Mat &frame)
{
  ...
  // Edge case when face exits frame
  ...
  // If the detected "face" is malformed, reject it.
  double ar = (double)m_faceTemplate.cols / m_faceTemplate.rows;
  if (ar < 0.6 || ar > 1.1) {
    setFaceLost();
    m_templateMatchingRunning = false;
    m_templateMatchingStartTime = m_templateMatchingCurrentTime = 0;
...
cv::Point VideoFaceDetector::getFrameAndDetect(cv::Mat &frame)
{
  *m_videoCapture >> frame;
  ...
  // Camera shut off or input file finished.
  if (frame.empty()) {
    m_facePosition.x = m_facePosition.y = 0;
